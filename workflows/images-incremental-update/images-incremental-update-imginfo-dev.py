...
fields_list = [("info","all_cdr_ids"), ("info","s3_url"), ("info","all_parent_ids"), ("info","image_discarded"), ("info","cu_feat_id"), ("info","img_info")]
...
def to_s3_url_key_dict_list(data):
    ...
    if "info:obj_stored_url" in v:
        ...
        if s3url.startswith('https://s3'):
            ...
            # put v as a list so we can extend it when we reduce in reduce_s3url_infos
            tup_list = [(s3url, [v])]
    return tup_list


def s3url_dict_list_to_cdr_id_wsha1(data):
    """ Transforms data expected to be in format (s3_url, (v, sha1)) into a list 
    of tuples (doc_id,v) where v contains the sha1.
    """
    ...
    sha1 = data[1][1]
    tup_list = []
    # for each doc_id with this s3_url
    for v in list_v:
...
def split_sha1_kv_images_discarded_wimginfo(x):
    ...
    tmp_fields_list = [("info","all_cdr_ids"), ("info","s3_url"), ("info","all_parent_ids"), ("info","img_info")]
    out = []
...
def out_to_amandeep_dict_str_wimginfo(x):
    ...
    for field in ["all_parent_ids", "s3_url", "all_cdr_ids", "cu_feat_id", "img_info"]:
...
def get_update_join_rdd(basepath_save, es_man, es_ts_start, es_ts_end, hbase_man_cdrinfos_out, hbase_man_update_out, incr_update_id, nb_partitions, c_options):
    ...
    sha1_infos_rdd_from_join = cdr_ids_infos_rdd_join_sha1.flatMap(cdrid_key_to_sha1_key_wimginfo)
    update_join_rdd = sha1_infos_rdd_from_join.reduceByKey(reduce_sha1_infos_discarding_wimginfo)
    ...
    if c_options.restart:
        try:
            if hdfs_file_exist(out_join_rdd_path):
                out_join_rdd_amandeep = sc.sequenceFile(out_join_rdd_path).mapValues(amandeep_dict_str_to_out_wimginfo)
        except Exception as inst:
            pass
        ...
        if out_join_rdd_amandeep is None or out_join_rdd_amandeep.isEmpty(): 
            ...
            try:
                if not hdfs_file_exist(out_join_rdd_path):
                    out_join_rdd_amandeep.filter(filter_out_rdd).map(out_to_amandeep_dict_str_wimginfo).saveAsSequenceFile(out_join_rdd_path)
                else:
...
def get_cdr_ids_infos_rdd_new_sha1(basepath_save, es_man, es_ts_start, es_ts_end, hbase_man_cdrinfos_out, hbase_man_update_out, incr_update_id, nb_partitions, c_options):
    ...
    if s3url_infos_rdd_join is not None:
        # format of s3url_infos_rdd_join is: (s3_url, ([v], sha1)) 
        # but actually sha1 is None for all urls if we did not join
        s3url_infos_rdd_with_sha1 = s3url_infos_rdd_join.filter(get_existing_joined_sha1)
...
def get_update_rdd(basepath_save, es_man, es_ts_start, es_ts_end, hbase_man_cdrinfos_out, hbase_man_update_out, incr_update_id, nb_partitions, c_options):
    ...
    update_rdd = cdr_ids_infos_rdd_new_sha1.flatMap(cdrid_key_to_sha1_key_wimginfo).reduceByKey(reduce_sha1_infos_discarding_wimginfo)
    ...
    if c_options.restart:
        ...
        try:
            if hdfs_file_exist(out_rdd_path):
                out_rdd_amandeep = sc.sequenceFile(out_rdd_path).mapValues(amandeep_dict_str_to_out_wimginfo)
        except Exception as inst:
    ...
    if c_options.save_inter_rdd:
        try:
            if not hdfs_file_exist(out_rdd_path):
                ...
                out_rdd_save = out_rdd_amandeep.filter(filter_out_rdd).map(out_to_amandeep_dict_str_wimginfo)
...
def save_out_rdd_to_hbase(out_rdd_amandeep, hbase_man_sha1infos_out):
    if out_rdd_amandeep is not None:
        ...
        out_rdd = out_rdd_amandeep.flatMap(split_sha1_kv_images_discarded_wimginfo)
