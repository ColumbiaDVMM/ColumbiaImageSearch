...
# TODO: if we use gunicorn with multiple processes we should output process id otherwise logs would be confusing
...
class APIResponder(Resource):
  ...
  def process_mode(self, mode):
    if mode == "status":
      ...
      return self.refresh()
    # TODO: add a backup mode that saves index to s3?
    else:
  ...
  def refresh(self):
    ...
    # TODO: get new features from some 'updater' that could read from kafka topic or from table_update in HBase...
