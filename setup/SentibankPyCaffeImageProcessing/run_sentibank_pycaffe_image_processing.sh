...
python ../../${package_name}/${package_name}/updater/extraction_checker.py -d -c ../../conf/${conf} &> log_check${suffix}_${extr_type}_$(date +%Y-%m-%d_%H-%M-%S).txt &
python ../../${package_name}/${package_name}/updater/extraction_processor.py -c ../../conf/${conf} &> log_proc${suffix}_${extr_type}_$(date +%Y-%m-%d_%H-%M-%S).txt
