    ...
    def get_ids_from_sha1s_hbase(self, list_sha1s):
        ...
        if list_sha1s:
            try:
                ...
                print("[HBaseIndexer.get_ids_from_sha1s_hbase] caught timeout error. Trying to refresh connection pool.")
                self.pool = happybase.ConnectionPool(size=self.nb_threads,host=self.hbase_host)
    ...
    def get_full_sha1_rows(self, list_sha1s):
        ...
        if list_sha1s:
            try:
                ...
                print("[HBaseIndexer.get_full_sha1_rows] caught timeout error. Trying to refresh connection pool.")
                self.pool = happybase.ConnectionPool(size=self.nb_threads,host=self.hbase_host)
    ...
    def build_extractions_rows(self, new_files_id, extractions_names, extractions_vals):
        ...
        for i, sha1 in enumerate(new_files_id):
            ...
            for i, extr in enumerate(extractions_names):
                row_data[extractions_columns_names[i]] = base64.b64encode(feats[i])
                row_data[extractions_columns_names[i]] = base64.b64encode(hashcodes[i])
