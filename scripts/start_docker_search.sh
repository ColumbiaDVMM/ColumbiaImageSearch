...
docker_name=${conf_name}"_search"
start_script=${indocker_repo_path}"/scripts/run_search.sh"
...
## Start process
echo "Starting Sentibank Search"
${SUDO} docker exec -itd ${docker_name} bash ${start_script} -r ${indocker_repo_path} -c ${conf_name} -e ${endpoint}