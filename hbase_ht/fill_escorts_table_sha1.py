...
def get_sha1(tab_sha1_name,rows_ids):
    ...
    with pool.connection() as connection:
        ...
        rows_sha1 = tab_sha1.rows(rows_ids,columns=["meta:sha1","meta:columbia_near_dups_sha1","meta:columbia_near_dups_sha1_dist"])
...
def get_image_ids_url(batch):
    ...
    for i,row in enumerate(batch):
        ...
        if iid:
            list_images_ids_url.append([row[0].strip(),iid])
...
def fill_sha1_info_onebatch(tab_escimg_name,tab_sha1_name,batch_size,start_row):
    batch,c_read,new_start_row = read_batch(tab_escimg_name,start_row,batch_size)
    if c_read>0:
        print "Read {} rows. Processing.".format(c_read)
        sys.stdout.flush()
        list_images_ids_url = get_image_ids_url(batch)
        print "Got {} image ids.".format(len(list_images_ids_url))
        image_ids = [x[1] for x in list_images_ids_url]
        #print image_ids
        sha1s_list = get_sha1(tab_sha1_name,image_ids)
        #print sha1s_list
        # sha1s_list[pos]=[im_sha1,nd_sha1,ndd_sha1]
        ...
        out_batch = [(x[0],{"info:sha1": sha1s_list[i][0]}) for i,x in enumerate(list_images_ids_url) if sha1s_list[i] and sha1s_list[i]!=[None,None,None]]
        print "Saving {} rows.".format(len(out_batch))
        print out_batch
...
def fill_sha1_info(tab_escimg_name,tab_sha1_name,batch_size,start_row):
    ...
    if count_total>0:
        print "Read {} rows total. Average time per row is: {}. Total time is: {}.".format(count_total,tel/count_total,tel)
